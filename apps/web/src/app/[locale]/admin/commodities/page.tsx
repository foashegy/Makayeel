...
export default async function AdminCommoditiesPage({
  ...
  const rows = await prisma.commodity.findMany({ orderBy: { displayOrder: 'asc' } });

  return (
    <div className="mx-auto max-w-content px-6 py-10">
      <h1 className="mb-8 text-3xl font-medium text-deep-navy">{t('commoditiesTitle')}</h1>
        ...
        <table className="w-full text-sm">
          <thead className="border-b border-navy/8 bg-cream/60 text-xs uppercase text-navy-300">
            <tr>
              ...
              <th className="px-4 py-3 text-start">{locale === 'ar' ? 'الفئة' : 'Category'}</th>
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.id} className="border-b border-navy/6 last:border-0">
                ...
                <td className="px-4 py-3 text-xs text-navy-300">{r.category}</td>
      ...
      <p className="mt-4 text-xs text-navy-200">
        {locale === 'ar' ? 'CRUD كامل هيتضاف قريبًا — دلوقتي قراءة فقط.' : 'Full CRUD coming soon — read-only for now.'}
      </p>
    </div>
  );
}
